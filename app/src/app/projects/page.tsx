          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filteredProjects.map((project, index) => (
                <FadeIn key={project.id} delay={index * 100} className={index === 0 ? "md:col-span-2" : ""}>
                  <Link
                    href={`/projects/${project.slug}`}
                    className="group relative rounded-2xl sm:rounded-3xl overflow-hidden cursor-pointer block h-full"
                  >
