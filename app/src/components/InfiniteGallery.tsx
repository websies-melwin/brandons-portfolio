...
function GalleryScene({
	...
	// Reset when resetProgress changes
	useEffect(() => {
		if (resetProgress) {
		...
		(event: KeyboardEvent) => {
			...
			if (event.key === 'ArrowUp' || event.key === 'ArrowLeft') {
				...
				if (!hasCompletedRef.current) {
					totalScrollDistanceRef.current += 2 * speed * 10;
					if (totalScrollDistanceRef.current >= oneRotationDistance * REQUIRED_ROTATIONS) {
						hasCompletedRef.current = true;
						onScrollCompleteRef.current?.();
					}
	...
	useFrame((state, delta) => {
		if (autoPlay) {
			// Only add velocity if below a cap to prevent indefinite accumulation
			setScrollVelocity((prev) => {
				if (prev < 2) {
					return prev + 0.5 * delta;
				}
				return prev;
			});
		}

		// Apply damping and clamp to reasonable range
		setScrollVelocity((prev) => {
			const damped = prev * 0.95;
			// Clamp velocity to prevent extreme values
			return Math.max(-10, Math.min(10, damped));
		});
	...
	useEffect(() => {
		const checkWebGL = () => {
			try {
				...
				if (gl) {
					const isContextLost = gl instanceof WebGLRenderingContext && gl.isContextLost();
					if (!isContextLost) {
		...
		<div className={className} style={style}>
			<Canvas 
				...
				onCreated={({ gl }) => {
					...
					// Handle context loss - force remount
